...
#define THIS Thing
	...
	template <typename T>
	wbool THIS::isSuper() const { return getClass().isSuper(T::getStaticClass()); }
	...
	template <typename T>
	wbool THIS::isSub() const { return T::getStaticClass().isSuper(getClass()); }
	//	to는 명시적캐스팅이다. 
	...
	template <typename T>
	TRefer<T> THIS::to() { return TRefer<T>(to(T::getStaticClass())); }
	template <typename T>
	TRefer<T> THIS::to() const { return TRefer<T>(to(T::getStaticClass())); }
	//	Casting:
	...
	//	invisible이다.
	template <typename T>
	T& THIS::cast() { return _down(T::getStaticClass()); }
	template <typename T>
	const T& THIS::cast() const { return _cast(T::getStaticClass()); }
	Refer THIS::toImplicitly(const Class& cls)
	{
		if(isSub(cls))
			return Refer(*this);
		return Refer();
	}
	Refer THIS::toImplicitly(const Class& cls) const
	{
		This& unconst = const_cast<This&>(*this);
		return Refer((const Refer&) unconst.toImplicitly(cls));
		// we code inliney because of prevention to releasing instance of returned variable from unconst.toImplicitly()
	}
	//	가상할당자이다. 할당연산자는 virtual이 안되기 때문에 제대로 할당을 하고 싶다면 항상 구체타입을 알고 있어야만 한다.
	Result& THIS::assign(const Thing& it) { if(it.isNull()) return NullError; }

	//	Visitor에 의해서 하위 구성요소(ownee)들을 어떻게 순회시킬지를 정한다.
	Result& THIS::_tour(Visitor& visitor) const { return Success; }
	Result& THIS::_tour(Visitor& visitor)
	{
