	...
	{		
		...
		if(look.y >= 0)
			...
			if(look.z >= 0)
				...
				division = 2; //2
		if(division == 2 || division == 3)	//	1,4 사분면이라면 a_y는 180-x도
			a_x = D3DX_PI - a_x;

		float cosx1 = cosf(a_x);
		a_y = asinf(look.x / cosx1);	//	이 시점에서 a_y는 2가지의 경우의 수가 있음. x도, 180-x도.
		//	x-z평면의 사분면 구하기
		if(look.x >= 0)
			if(look.z >= 0)
				division = 1;
			...
			if(look.z >= 0)
				...
				division = 3;
		if(division == 2 || division == 3)	//	2,3 사분면이라면 a_y는 180-x도
			a_y = D3DX_PI - a_y;
		a_z = D3DX_PI / 2.0f;
...
cx
	...
	*/	
	...
	g_pD3DDevice->BeginScene();
	{
		...
		d += 0.01f;
		D3DXMatrixRotationX(&wx, d);
		D3DXMatrixRotationY(&wy, 0);
