...
//	생성자
CEngine::CEngine() : _direct(NULL), _device(NULL),
...
void	CEngine::initializeMatrix()
{
	...
	D3DXMatrixOrthoLH(	&projection_matrix, 
						(float) g_ini.getScreenWidth(),
						(float) g_ini.getScreenHeight(),
...
void	CEngine::loadTexture(CString filename)
{
	if (_texture_size < _texture_capacity ) // _texture_capacity-1 : 최대 허용가능한 인덱스
	{
		...
		_texture[_texture_size].loadTexture(_device, g_ini._grpahic_32bit_texture);
		_texture_size++;		
	}
	else
	{
...
{		
	_shader.initialize(_device, &g_ini);
}
