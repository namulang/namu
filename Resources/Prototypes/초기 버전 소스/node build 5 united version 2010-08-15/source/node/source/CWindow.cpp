...
CWindow::CWindow(HINSTANCE hInstance, int nCmdShow, CScripter* scripter) 
: _hInstance(hInstance), _nCmdShow(nCmdShow), _scripter(scripter)
{	
	//	초기화:
	isAlreadyExcuted();
...
void	CWindow::setClientRect()
{
	...
	SetRect(&crt, 0, 0, g_ini._graphic_width, g_ini._graphic_height); // RECT는 구조체이기에 이함수로 초기화시킨다
