...
*/
```
* [x] namulang은 비어있는 블록문을 가능하면 피하고 싶습니다.<br/>꼭
...
=@abe
```
* [x] abe는 프로퍼티를 정의하는게 아니므로 프로퍼티 명1개만 필요합니다.
* [x] 그러니 `@abe`로 프로퍼티명과 타입을 같이 서술하고자 하는게 의미가 없습니다.
* * *
## 클래스라는 건 없습니다
...
outerFunc(p p1) str // 에러! p1은 어디까지나 main() 안에서 정의되었으니 밖에서는 모릅니다.
  p.name
```
* [x] 이처럼 namulang은 클래스와 일반 객체간 경계가 거의 없습니다.
...
* [x] 하지만 타입 A가 있을때, 별칭 B를 만들고 싶으면, 다른 언어에서는 새로운 키워드를 사용해야 합니다. 예를들어,
```cpp
typedef A B;
```
...
* * *
## err
* [x] namulang에 에러는 개념적으로 2가지로 분류됩니다.
