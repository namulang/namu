...
#define SPRINT(n) \
    NM_W("%d times new/delete : %f ms elapsed. crc=%s", n, ((float) run1(crc, (n))) / CLOCKS_PER_SEC*1000.0f, crc); \
    NM_W("%d times mempool    : %f ms elapsed. crc=%s", n, ((float) run2(crc, (n))) / CLOCKS_PER_SEC*1000.0f, crc);
