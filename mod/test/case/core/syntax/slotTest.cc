...
TEST_F(slotTest, slotIsInFrameWhenCallMgdFunc) {
    ...
    f1.setLambda([](const auto& contain, const auto& sf) {
        const frame& fr = getOr(sf[sf.len() - 1]) orRet false;

        // checks slot is in frame:
        const myfunc& cast = getOr(fr.sub<myfunc>("foo", narr(nInt(), nFlt()))) orRet false;
        const params& ps = getOr(cast.getParams()) orRet false;
        ...
        const nInt& age = getOr(fr.sub<nInt>("age")) orRet false;
        if(age.cast<int>() != 1) return false;

        const nFlt& grade = getOr(fr.sub("grade").cast<nFlt>()) orRet false;
