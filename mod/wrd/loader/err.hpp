...
namespace wrd {
    ...
    public:
        void log() const override {
            switch(fType) {
                case ERR: WRD_E("%d:%d: err: %s", srcArea.start.row, srcArea.start.col, msg.c_str()); break;
                case WARN: WRD_W(":%d:%d: warn: %s", srcArea.start.row, srcArea.start.col, msg.c_str()); break;
                case INFO: WRD_I(":%d:%d: info: %s", srcArea.start.row, srcArea.start.col, msg.c_str()); break;
            }
        }
	};
}
