...
#define LOG(...) if(isLog()) NAMU_DI(__VA_ARGS__)
    ...
    nbool me::onVisit(visitInfo i, node& me) {
        ...
        for(auto e=me.subs().begin(); e ;++e) {
            if(me.subAll<baseObj>(e.getKey()).len() > 1)
                return _err(e->getPos(), errCode::DUP_VAR, e.getKey().c_str()), true;
        }
        return true;
    }
    ...
    nbool me::onVisit(visitInfo i, asExpr& me) {
        ...
        if(nul(me.getMe())) return _err(me.getPos(), errCode::LHS_IS_NULL), true;
        if(nul(me.getAs())) return _err(me.getPos(), errCode::RHS_IS_NULL), true;
        if(me.getAs().isSub<nVoid>()) return _err(me.getPos(), errCode::VOID_NOT_CAST), true;
        ...
        if(!me.getAs().asImpli<node>())
            return _err(me.getPos(), errCode::CAST_TO_UNKNOWN), true;
        return true;
    }
    ...
    nbool me::onVisit(visitInfo i, assignExpr& me) {
        ...
        if(!leftEval) return _err(me.getPos(), errCode::LHS_IS_NULL), true;
        const ntype& ltype = leftEval->getType();
        if(nul(ltype)) return _err(me.getPos(), errCode::LHS_IS_NULL), true;
        str rightEval = me.getRight().getEval();
        if(!rightEval) return _err(me.getPos(), errCode::RHS_IS_NULL), true;
        const ntype& rtype = rightEval->getType();
        if(nul(rtype)) return _err(me.getPos(), errCode::RHS_IS_NULL), true;
    ...
    nbool me::onVisit(visitInfo i, defPropExpr& me) {
        ...
        str eval = me.getEval();
        if(!eval)
            return _err(me.getPos(), errCode::TYPE_NOT_EXIST, me.getName().c_str()), true;
        if(eval->isSub<nVoid>())
            return _err(me.getPos(), errCode::VOID_CANT_DEFINED), true;
        ...
        for(const node& bean : beans)
            if(bean.isSub<nVoid>())
                return _err(me.getPos(), errCode::NO_VOID_CONTAINER), true;

        const ntype& t = eval->getType();
        const nchar* typeName = nul(t) ? "null" : t.getName().c_str();
        if(nul(top)) return true;
        if(top.has(me.getName()))
            return _err(me.getPos(), errCode::ALREADY_DEFINED_VAR, me.getName().c_str(), typeName), true;


        // 'check duplication' must be front of 'is %s definable':
        std::string name = me.getName();
        LOG("verify: defPropExpr: is %s definable?", name.c_str());
        if(name == "") return _err(me.getPos(), errCode::HAS_NO_NAME), true;
        const node& org = me.getOrigin();
        if(nul(org)) return _err(me.getPos(), errCode::NO_ORIGIN, name.c_str()), true;
    ...
    nbool me::onVisit(visitInfo i, defSeqExpr& me) {
        ...
        if(nul(me.getStart())) return _err(me.getPos(), errCode::LHS_IS_NULL), true;
        if(nul(me.getEnd())) return _err(me.getPos(), errCode::RHS_IS_NULL), true;

        LOG("verify: defSeqExpr: lhs & rhs is sort of Int?");
        if(!me.getStart().isImpli<nInt>()) return _err(me.getPos(), errCode::SEQ_SHOULD_INT_COMPATIBLE), true;
        if(!me.getEnd().isImpli<nInt>()) return _err(me.getPos(), errCode::SEQ_SHOULD_INT_COMPATIBLE), true;
        return true;
    }
    ...
    nbool me::onVisit(visitInfo i, defArrayExpr& me) {
        ...
        if(nul(type)) return _err(me.getPos(), errCode::ELEM_TYPE_DEDUCED_NULL), true;
        if(type.isSuper<obj>())
            return _err(me.getPos(), errCode::ELEM_TYPE_DEDUCED_WRONG, type.getType().getName().c_str()), true;
        if(type.isSub<nVoid>())
            return _err(me.getPos(), errCode::ELEM_TYPE_NOT_VOID), true;
        return true;
    }
    ...
    nbool me::onVisit(visitInfo i, FBOExpr& me) {
        ...
        const node& lhs = me.getLeft();
        const node& rhs = me.getRight();
        if(nul(lhs)) return _err(me.getPos(), errCode::LHS_IS_NULL), true;
        if(nul(rhs)) return _err(me.getPos(), errCode::RHS_IS_NULL), true;

        LOG("verify: FBOExpr: finding eval of l(r)hs.");
        str lEval = lhs.getEval();
        str rEval = rhs.getEval();
        if(!lEval) return _err(me.getPos(), errCode::LHS_IS_NULL), true;
        if(!rEval) return _err(me.getPos(), errCode::RHS_IS_NULL), true;

        if(!checkEvalType(*lEval)) return _err(me.getPos(), errCode::LHS_IS_NOT_ARITH, lEval->getType().getName().c_str()), true;
        if(!checkEvalType(*rEval)) return _err(me.getPos(), errCode::RHS_IS_NOT_ARITH, rEval->getType().getName().c_str()), true;
        ...
        if((lEval->isSub<nStr>() || rEval->isSub<nStr>())) {
            switch(r) {
                ...
                case FBOExpr::BITWISE_OR: case FBOExpr::LSHIFT: case FBOExpr::RSHIFT:
                    return _err(me.getPos(), errCode::STRING_IS_NOT_PROPER_TO_OP), true;
    ...
    nbool me::onVisit(visitInfo i, getExpr& me) {
        ...
        if(!me.getEval()) return _err(me.getPos(), errCode::WHAT_IS_THIS_IDENTIFIER, me.getSubName().c_str()), true;
        auto matches = me._get(true);
        if(matches.isEmpty()) {
            const node& from = me.getMe();
            return _err(me.getPos(), errCode::CANT_ACCESS, me._name.c_str(), from.getType().getName().c_str()), true;
        }
        if(matches.len() >= 2) {
            // TODO: leave all ambigious candidates as err.
            return _err(me.getPos(), errCode::AMBIGIOUS_ACCESS, i.name.c_str()), true;
        }
        ...
        if(asedMe && !asedMe->isComplete())
            return _err(me.getPos(), errCode::ACCESS_TO_INCOMPLETE), true;
        return true;
    }
    ...
    nbool me::onVisit(visitInfo i, retExpr& me) {
        ...
        if(nul(f)) return _err(me.getPos(), errCode::NO_FUNC_INFO), true;

        str myEval = me.getEval();
        if(!myEval) return _err(me.getPos(), errCode::EXPR_EVAL_NULL), true;
        ...
        if(!myType.isImpli(fType))
            return _err(me.getPos(), errCode::RET_TYPE_NOT_MATCH, myType.getName().c_str(), fType.getName().c_str()), true;
        return true;
    }

    nbool me::onVisit(visitInfo i, runExpr& me) {
        LOG("verify: runExpr: is it possible to run?");
        if(nul(me.getMe())) return _err(me.getPos(), errCode::DONT_KNOW_ME), true;

        str ased = me.getMe().getEval();
        if(!ased) return _err(me.getPos(), errCode::DONT_KNOW_ME), true;

        node& anySub = me.getSubject();
        if(nul(anySub)) return _err(me.getPos(), errCode::FUNC_NOT_EXIST), true;
        ...
        if(!derivedSub) return _err(me.getPos(), errCode::CANT_ACCESS, ased->getType().getName().c_str(), "sub-node"), true;
        ...
        if(!derivedSub->canRun(me.getArgs())) {
            ...
            return _err(me.getPos(), errCode::OBJ_WRONG_ARGS, i.name.c_str(), me.getArgs().asStr().c_str(), params.c_str()), true;
        }
    ...
    nbool me::onVisit(visitInfo i, func& me) {
        ...
        if(nul(meObj)) return _err(me.getPos(), errCode::FUNC_REDIRECTED_OBJ), true;
        ...
        if(!retType) return _err(me.getPos(), errCode::NO_RET_TYPE), true;
        if(!retType->isSub(ttype<node>::get()))
            return _err(me.getPos(), errCode::WRONG_RET_TYPE, retType->getType().getName().c_str()), true;
        ...
        if(lastStmt.isSub<retStateExpr>())
            return _err(lastStmt.getPos(), errCode::FUNC_SHOULD_RETURN_SOMETHING), true;
    ...
    nbool me::onVisit(visitInfo i, forExpr& me) {
        ...
        if(!conAsed)
            return _err(me.getPos(), errCode::CONTAINER_IS_NULL), true;
        str elemType = conAsed->run("getElemType");
        if(!elemType)
            return _err(me.getPos(), errCode::ELEM_TYPE_IS_NULL), true;
    ...
    nbool me::onVisit(visitInfo i, breakExpr& me) {
        ...
        if(_recentLoops.size() <= 0) return _err(me.getPos(), errCode::BREAK_OUTSIDE_OF_LOOP), true;
        return true;
    }

    nbool me::onVisit(visitInfo i, nextExpr& me) {
        LOG("verify: nextExpr: declared outside of loop?");
        if(_recentLoops.size() <= 0) return _err(me.getPos(), errCode::NEXT_OUTSIDE_OF_LOOP), true;
        return true;
    }
    ...
    nbool me::onVisit(visitInfo i, ifExpr& me) {
        ...
        if(nul(condition))
            return _err(me.getPos(), errCode::CONDITION_IS_EMPTY), true;
        if(!condition.is<nBool>())
            return _err(me.getPos(), errCode::CONDITION_CANT_CAST_TO_BOOL), true;
        return true;
    }
}
