...
namespace nm {
    ...
    nint tokenScan::onScan(parser& ps, YYSTYPE* val, YYLTYPE* loc, yyscan_t scanner, nbool& isBypass) {
        ...
        NM_DI("%s: dispatcher[%s]%s(token: \"%s\" [%s]) at %s,%s", *this, disp.len(), isBypass ? ".dispatch" : " lowscanner",
                tokName, tok, loc->start.row, loc->start.col);
        return tok;
    }
    ...
    nint indentScan::onScan(parser& ps, YYSTYPE* val, YYLTYPE* loc, yyscan_t scanner, nbool& isBypass) {
        ...
        if(ind.size() == 0) {
            NM_DI("indentScan: initial indent lv: %s", cur);
            ind.push_back(cur);
        }

        ncnt prev = ind.back();
        NM_DI("indentScan: column check: cur[%s] prev[%s]", cur, prev);
