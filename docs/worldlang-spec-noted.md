...
* T가 구체타입이나 디폴트 생성자가 없을 경우가 문제다.
    ...
    * T가 만약 native에서 정의된 구체타입일 경우: 별다른 조치를 취하지 않으면 WRD_CLASS 매크로 안쪽에서 에러가 발생할 것이다.
        * 만약 개발자가 어떻게든 디폴트 생성자 없이 WRD_CLASS를 쓰고 싶다고 한다면 직접 WRD_CLASS 대체하는 코드를 넣어줄 수 밖에 없다.
