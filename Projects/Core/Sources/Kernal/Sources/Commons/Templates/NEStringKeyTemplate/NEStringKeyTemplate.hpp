#pragma once

#include "../NEKeyTemplate/NEKeyTemplate.hpp"
#include "../../../Modules/NEString/NEString.hpp"
#include "../../../Modules/NEWString/NEWString.hpp"

namespace NE
{
	template <typename T, NEType::Type type>
	class NEStringKeyTemplate : public NEKeyTemplate<T, type>
	{
	public:
		typedef NEStringKeyTemplate ThisClass;
		typedef NEKeyTemplate<T, type> SuperClass;

	public:
		NEStringKeyTemplate(const T& source = T(), const NETString& keyname = "");
		NEStringKeyTemplate(const ThisClass& source);
		NEStringKeyTemplate(const NEKey& source);

	public:
		virtual type_result assign(const NEKey& source);
		virtual bool isEqualTo(const NEKey& source) const;
		virtual type_result add(NEKey& result, const NEKey& operand2) const;
		virtual bool isLower(const NEKey& source) const;

	public:
		virtual NEObject& clone() const;
	};
}

#include "NEStringKeyTemplate.inl"